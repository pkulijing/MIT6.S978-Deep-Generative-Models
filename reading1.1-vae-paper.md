...
4. **两个优化目标**：
   ...
   - **重构项**: $\mathbb{E}_{q_\phi(z|x)}[\log p_\theta(x|z)]$ — 确保解码器能从 $z$ 重构出 $x$
...
| 项目         | KL 散度项                              | 重构项                                           |
| ------------ | -------------------------------------- | ------------------------------------------------ |
| **理论形式** | $D_{KL}(q_\phi(z\|x) \| p_\theta(z))$  | $\mathbb{E}_{q_\phi(z\|x)}[\log p_\theta(x\|z)]$ |
...
| 步骤              | 表达式                                                            | 含义                                           |
| ----------------- | ----------------------------------------------------------------- | ---------------------------------------------- |
| **1**             | $\nabla_\phi \mathbb{E}_{q_\phi(z\|x)}[f(z)]$                     | 期望的梯度——难以直接计算                       |
| **2 (REINFORCE)** | $= \mathbb{E}_{q_\phi(z\|x)}[f(z) \nabla_\phi \log q_\phi(z\|x)]$ | 重写为另一个期望（高方差）                     |
| **3 (重参数化)**  | $= \mathbb{E}_{p(\epsilon)}[\nabla_\phi f(g_\phi(x, \epsilon))]$  | 重新参数化——从固定噪声采样（**低方差**和可微） |
