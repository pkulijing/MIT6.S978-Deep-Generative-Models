...
- [2. 论文的数学基础](#2-论文的数学基础)
  ...
  - [2.3 二维卷积与因果卷积](#23-二维卷积与因果卷积)
    - [2.3.1 什么是二维卷积](#231-什么是二维卷积)
    - [2.3.2 二维卷积可视化](#232-二维卷积可视化)
    - [2.3.3 因果卷积 (Causal Convolution)](#233-因果卷积-causal-convolution)
...
- [3. 论文的主要逻辑](#3-论文的主要逻辑)
  - [3.1 像素生成的自回归分解](#31-像素生成的自回归分解)
    - [3.1.1 联合分布分解](#311-联合分布分解)
    - [3.1.2 RGB通道的条件依赖](#312-rgb通道的条件依赖)
    - [3.1.3 256-way Softmax建模离散像素值](#313-256-way-softmax建模离散像素值)
  - [3.2 Row LSTM架构](#32-row-lstm架构)
    - [3.2.1 核心思想：从"逐像素"到"逐行"的建模](#321-核心思想从逐像素到逐行的建模)
    - [3.2.2 Input-to-state 和 State-to-state 卷积](#322-input-to-state-和-state-to-state-卷积)
    - [3.2.3 三角形感受野的形成](#323-三角形感受野的形成)
    - [3.2.4 并行化优化](#324-并行化优化)
  - [3.3 Diagonal BiLSTM架构](#33-diagonal-bilstm架构)
    - [3.3.1 三角形感受野问题成因](#331-三角形感受野问题成因)
    - [3.3.2 Skewing操作](#332-skewing操作)
    - [3.3.3 双向处理与因果性](#333-双向处理与因果性)
    - [3.3.4 PixelCNN架构](#334-pixelcnn架构)
    - [3.3.5 MultiScale PixelRNN](#335-multiscale-pixelrnn)
  - [3.4 Masked Convolution](#34-masked-convolution)
    - [3.4.1 引入Mask的原因](#341-引入mask的原因)
    - [3.4.2 Mask 定义](#342-mask-定义)
    - [3.4.3 网络的整体结构](#343-网络的整体结构)
- [4. 总结](#4-总结)
  - [4.1 论文的主要贡献](#41-论文的主要贡献)
  - [4.2 相比Bengio 1999的进步](#42-相比bengio-1999的进步)
  - [4.3 引出与第三篇论文(IAF)的联系](#43-引出与第三篇论文iaf的联系)
    - [4.3.1 PixelRNN的局限性](#431-pixelrnn的局限性)
    - [4.3.2 为什么自回归模型采样慢](#432-为什么自回归模型采样慢)
...
- **内容门** (Content gate): $\tilde{c}\_t = \tanh(W\_c \cdot [h\_{t-1}, x\_t] + b\_c)$ （生成候选的新信息）
...
$$
\begin{aligned}
x_1,x_2,\cdots,x_{i-1} &\rightarrow x_i^{\mathrm{pred}} \\\\
...
$$
\begin{aligned}
x_1,x_2,\cdots,x_{n^2} &\rightarrow x_1^{\mathrm{pred}},x_2^{\mathrm{pred}},\cdots,x_{n^2}^{\mathrm{pred}} \\\\
