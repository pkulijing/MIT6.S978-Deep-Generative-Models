...
$$
P(Z_i = 1|Z_1...Z_{i-1}) = \frac{1}{1 + \exp(-w_0 - \sum_{j<i} w_j Z_j)}=\sigma\left(w_0+\sum_{j<i}w_jZ_j\right)
$$
...
当采用完全连接的"左到右"结构时， $\text{Parents}_i = \{Z_1, ..., Z_{i-1}\}$ ，此时联合分布等价于链式法则的完全展开。Bengio在这里完成了图模型与AR模型的关联。
...
1. **隐藏层约束**：第 $j$ 组隐藏单元 $h_j$ 只接收来自 $k < j$ 的输入（即 $z_1, ..., z_{j-1}$）
2. **输出层约束**：第 $i$ 组输出 $g_i$ 只依赖于 $h_1, ..., h_i$（进而只依赖于 $z_1, ..., z_{i-1}$）

数学上表示为：

$$
\begin{aligned}h_{j,j'} &= \tanh(c_{j,j'} + \sum_{k<j} \sum_{k'=1}^{n_k} v_{j,j',k,k'} \cdot z_{k,k'}) & \text{中间层的激活值} \\
...
2. **概率估计/打分任务**（一次性前向传播）：
   ...
   - 输入完整序列 $(z_1, ..., z_n)$
   - 计算对数似然：$\log P(z) = \sum_{i=1}^{n} \log g_{i, z_i}$
   - 用于异常检测、分类等任务（**论文主要场景**）
...
使得对所有 $x \in [0,1]^d$ 都有 $|f(x) - \hat{f}(x)| < \epsilon$。
