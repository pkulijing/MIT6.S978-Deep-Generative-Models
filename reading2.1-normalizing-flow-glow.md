...
- [概述](#概述)
  - [核心思想](#核心思想)
  - [关键优势](#关键优势)
  - [变量变换公式的深层含义](#变量变换公式的深层含义)
    - [数学证明](#数学证明)
    - [几何直观理解](#几何直观理解)
    - [直观理解](#直观理解)
  - [与其他生成模型的对比](#与其他生成模型的对比)
- [第一篇论文：Variational Inference with Normalizing Flows (Rezende & Mohamed, 2015)](#第一篇论文variational-inference-with-normalizing-flows-rezende--mohamed-2015)
  - [1. 动机：为什么需要归一化流？](#1-动机为什么需要归一化流)
    - [变分推断的核心困境](#变分推断的核心困境)
    - [Mean-Field近似的致命缺陷](#mean-field近似的致命缺陷)
    - [归一化流的解决方案](#归一化流的解决方案)
  - [2. 核心概念](#2-核心概念)
    - [2.1 变量变换的数学基础](#21-变量变换的数学基础)
    - [2.2 有限流（Finite Flows）](#22-有限流finite-flows)
    - [Planar Flow（平面流）](#planar-flow平面流)
    - [Radial Flow（径向流）](#radial-flow径向流)
    - [径向流雅可比行列式的详细推导](#径向流雅可比行列式的详细推导)
    - [直观理解：重力场类比](#直观理解重力场类比)
    - [2.3 无限小流（Infinitesimal Flows）：理论极限的力量](#23-无限小流infinitesimal-flows理论极限的力量)
    - [核心思想：从离散到连续](#核心思想从离散到连续)
    - [Langevin流：通往任意分布的桥梁](#langevin流通往任意分布的桥梁)
    - [Hamiltonian流：另一种视角](#hamiltonian流另一种视角)
    - [对实践者的关键要点](#对实践者的关键要点)
  - [3. 与变分推断的结合](#3-与变分推断的结合)
    - [从VAE到Flow-ELBO的完整推导](#从vae到flow-elbo的完整推导)
    - [步骤1：标准VAE的ELBO](#步骤1标准vae的elbo)
    - [步骤2：引入归一化流](#步骤2引入归一化流)
    - [步骤3：展开KL散度](#步骤3展开kl散度)
    - [步骤4：关键变换 - 合并为联合概率](#步骤4关键变换---合并为联合概率)
    - [步骤5：转换到基础分布 $q_0$](#步骤5转换到基础分布-q_0)
    - [步骤6：用变量变换公式表达 $q_K$](#步骤6用变量变换公式表达-q_k)
    - [步骤7：最终的Flow-ELBO](#步骤7最终的flow-elbo)
    - [理解三项的来源](#理解三项的来源)
    - [关键要点总结](#关键要点总结)
    - [$p(z_K)$的两种形式](#pz_k的两种形式)
    - [摊销推断（Amortized Inference）](#摊销推断amortized-inference)
  - [4. 实验结果分析](#4-实验结果分析)
    - [2D合成数据的直观理解](#2d合成数据的直观理解)
    - [MNIST和CIFAR-10定量结果](#mnist和cifar-10定量结果)
  - [5. 关键贡献总结](#5-关键贡献总结)
    - [理论贡献](#理论贡献)
    - [实践贡献](#实践贡献)
- [第二篇论文：Glow (Kingma & Dhariwal, 2018)](#第二篇论文glow-kingma--dhariwal-2018)
  - [1. 动机：从数学玩具到实用生成模型](#1-动机从数学玩具到实用生成模型)
    - [第一篇论文留下的遗憾](#第一篇论文留下的遗憾)
    - [为什么第一篇的方法生成不了好图像？](#为什么第一篇的方法生成不了好图像)
    - [Glow的核心动机：让Flow真正能生成高质量图像](#glow的核心动机让flow真正能生成高质量图像)
    - [从第一篇到第二篇：技术路线的转变](#从第一篇到第二篇技术路线的转变)
    - [Glow要解决的具体问题](#glow要解决的具体问题)
    - [直观对比：第一篇 vs Glow](#直观对比第一篇-vs-glow)
    - [阅读笔记的脉络](#阅读笔记的脉络)
  - [2. 架构组件深入分析](#2-架构组件深入分析)
    - [2.1 ActNorm层：数据适应的归一化](#21-actnorm层数据适应的归一化)
    - [为什么BatchNorm在此处失效？](#为什么batchnorm在此处失效)
    - [ActNorm的解决方案](#actnorm的解决方案)
    - [2.2 可逆1×1卷积：学习最优置换](#22-可逆11卷积学习最优置换)
    - [从固定到可学习](#从固定到可学习)
    - [对数行列式的计算挑战](#对数行列式的计算挑战)
    - [LU分解优化](#lu分解优化)
    - [2.3 仿射耦合层：聪明的"半数保持"策略](#23-仿射耦合层聪明的半数保持策略)
    - [核心矛盾：既要复杂又要可逆](#核心矛盾既要复杂又要可逆)
    - [仿射耦合层的精妙设计](#仿射耦合层的精妙设计)
    - [为什么这个设计如此巧妙？](#为什么这个设计如此巧妙)
    - [直观理解：信息的"单向流动"](#直观理解信息的单向流动)
    - [Glow相对于RealNVP的三大改进](#glow相对于realnvp的三大改进)
  - [3. 多尺度架构设计](#3-多尺度架构设计)
    - [层次化处理策略](#层次化处理策略)
    - [深度与宽度的平衡](#深度与宽度的平衡)
  - [4. 实验结果深度分析](#4-实验结果深度分析)
    - [4.1 消融实验：每个组件的贡献](#41-消融实验每个组件的贡献)
    - [1×1卷积 vs 固定置换](#11卷积-vs-固定置换)
    - [定量性能比较](#定量性能比较)
    - [4.2 定性结果：视觉质量的突破](#42-定性结果视觉质量的突破)
    - [高分辨率生成（256×256）](#高分辨率生成256256)
    - [温度采样的精妙控制](#温度采样的精妙控制)
    - [温度采样是什么？](#温度采样是什么)
    - [数学定义与直观理解](#数学定义与直观理解)
    - [不同温度的效果详解](#不同温度的效果详解)
    - [为什么 $T=0.7$ 是论文的推荐值？](#为什么-t07-是论文的推荐值)
    - [数学上的深层含义](#数学上的深层含义)
    - [实现细节](#实现细节)
    - [关键要点总结](#关键要点总结-1)
    - [语义属性操作](#语义属性操作)
