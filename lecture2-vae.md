...
- 第一项 $E_{x \sim p_\text{data}}[\log p_\text{data}(x)]$ 是数据分布的熵，**与参数 $\theta$ 无关**
...
- **重构损失**（Reconstruction Loss）: $-E_q[\log p_\theta(x|z)]$
  - 确保从 z 重构 x 的质量
...
- **损失函数**：重构损失 $|x-\mu_{k^{\star}}|^2$，其中 $k^{\star} = \argmin_k|x-\mu_k|^2$
...
2. **VQ Encoder（向量量化编码器）**：
  ...
  $$
    k^* = \underset{k}{\text{argmin}} |z_e - e_k|^2
  $$
